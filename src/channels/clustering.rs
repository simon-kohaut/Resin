...
pub fn frequency_adaptation(rc: &mut ReactiveCircuit, frequencies: &[f64], boundaries: &[f64]) {
    ...
    let mut bins = flip(&pack(&binning(&frequencies, boundaries)));

    for (index, cluster_step) in bins.iter_mut().enumerate() {
        if *cluster_step != 0 {
