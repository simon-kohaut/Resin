...
impl Manager {
    ...
    /// Returns a vector of the values of all leaves.
...
mod tests {
    ...
    fn test_read_write() -> Result<(), Box<dyn std::error::Error>> {
        ...
        // Before sending, value should still be 0.0
        assert_eq!(manager.get_values(), vec![0.0]);

        // Send a value
        writer.write(1.0, None);

        // Give the reader thread a moment to process the value
        sleep(Duration::from_millis(10));

        // Leaf should now have value 1.0.
        // Note: The update happens in a background thread.
        // In a real application, you'd need a way to wait for updates.
        // For this test, a small sleep is sufficient.
