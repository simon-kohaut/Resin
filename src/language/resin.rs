...
impl Resin {
    pub fn compile(model: &str) -> Result<Resin, Box<dyn std::error::Error>> {
    ...
    pub fn deploy(&mut self, target: usize) -> (Vec<DeployedCircuit>, Vec<f64>) {
    ...
    pub fn setup_signals(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for source in &self.sources {
            let index = self.manager.create_leaf(&source.name, 0.0, 0.0);
            self.manager.read(index as u16, &source.channel, false)?;

            let index = self
                .manager
                .create_leaf(&format!("-{}", source.name), 1.0, 0.0);
...
mod tests {
    ...
    //                 Some(writer) => {
    //                     writer.write(p_close_array.get(i).unwrap(), Some(simulation_time))
    //                 }
    //                 None => writers
    //                     .get(&(d2, d1))
    //                     .unwrap()
    //                     .write(p_close_array.get(i).unwrap(), Some(simulation_time)),
    //             };
    //         }

    //         // Make publish/subscribe cycle happen
    //         // With mpsc, this happens in background threads. We just need to prune.
